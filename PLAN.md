...
ARY 已完成 PRD / UX / 领域模型 / 本地 MVP、WEB-1 全角色 E2E / CI，以及 SEC-1 代码级安全基线。正式工程已具备 PostgreSQL migration、可撤销随机会话、OAuth state、Public DTO、CA HMAC / 防重放、安全响应头和生产配置门禁。
...
| --- | --- | --- | --- | --- |
| P0 | Hosted CI 首次验证 | 确认安全基线在远端环境可重复执行 | 静态检查、23 个领域测试、类型检查、PostgreSQL production build、生产配置预检和 14 个 E2E 均通过 | `.github/workflows/web-ci.yml`、`web/e2e/` |

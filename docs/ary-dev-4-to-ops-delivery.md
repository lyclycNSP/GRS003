...
当前实现入口：`../web/`
当前测试入口：`../web/tests/domain.test.ts`
...
本轮历史上曾在仓库中新增根目录 `app/` 本地 MVP 应用，把 `ary.plan.md` 中 `DEV-4` 到 `DEV-7`、`REL-1` 和 `OPS-1` 落为可运行、可测试、可复核的赛事闭环。当前工作树已删除该静态 MVP 源码；可运行闭环由 `../web/` 集成应用承接，本文保留为历史验收记录。
...
历史交付时仓库没有正式后端、数据库、OAuth、部署环境或生产CAConnector，因此当时“完整实现”按本地MVP口径完成。当前 `web/` 已接入 Next.js、Prisma、SQLite、OAuth fallback、Server Actions 和领域测试；生产发布仍需要后续接入真实生产基础设施。
...
| 历史文件 | 作用 |
|---|---|
| 根目录旧 `app/index.html` | 单页本地工作台，覆盖Overview、Race、CA、Screen、Reports、Public、Ops、Admin八个视图；当前文件已移除。 |
| 根目录旧 `app/styles.css` | 工作台视觉和响应式布局；当前文件已移除。 |
| 根目录旧 `app/domain.js` | 无DOM领域层，包含状态、选择器和全部赛事动作；当前语义迁入 `../web/lib/domain.ts`。 |
| 根目录旧 `app/app.js` | 浏览器渲染、表单、按钮动作、`localStorage`持久化；当前页面迁入 `../web/app/`。 |
| 根目录旧 `app/domain.test.js` | Node领域测试，覆盖关键验收边界；当前测试迁入 `../web/tests/domain.test.ts`。 |
| 根目录旧 `app/README.md` | 运行方式、验证命令和实现边界；当前文件已移除。 |
...
当前本地状态由 SQLite / Prisma seed 提供，调试角色入口见 `/debug-login`。
...
历史 `app/domain.js` 实现过以下动作；当前对应实现见 `../web/lib/domain.ts`：
...
历史 `app/index.html#race` 提供过以下入口；当前对应入口见 `../web/app/console/page.tsx` 和 `../web/app/races/[slug]/page.tsx`：
...
历史 `Screen Console` 位于 `app/index.html#screen`；当前对应入口见 `../web/app/screen/page.tsx`，支持：
...
历史 `domain.js` 实现过以下动作；当前对应实现见 `../web/lib/domain.ts`：
...
历史交付时已执行（以下命令引用的根目录旧 `app/` 已删除，不可在当前工作树直接运行）：

* JavaScript 语法检查：旧 `app/domain.js`、旧 `app/app.js`。
* 领域测试：旧 `app/domain.test.js`。
* 布局静态检查：旧 `app/layout-check.js`。
...
| 本地HTTP浏览器只读烟测 | 历史通过，曾确认根目录旧 `app/index.html` 可打开、标题为 `ARY Local MVP`、八个视图和 `Run P0` 按钮存在 |
...
浏览器验证边界：历史交付通过临时本地HTTP服务完成只读烟测；内置浏览器控制层在点击 `Run P0` 时超时，因此点击后的P0闭环以旧本地 MVP 领域测试作为可靠验收证据。当前回归以 `../web/tests/domain.test.ts` 为准。
...
* 选择正式应用技术栈。
* 继续将 `web/lib/domain.ts` 中的领域动作服务化或抽取为共享领域包。

...
* Organizer：查看并处置自己管理 Race 的全部风险；入口必须携带明确 `raceId`。
* Rider：跨赛事查看本人或所在团队的风险和整改建议；可重新打开相关风险。
* Judge：只查看已分配作品相关的风险和处置摘要；不修改风险状态。
* Admin：跨赛事查看全局治理摘要并筛选，保持只读，不能替代 Organizer 处置。
...
严重度兼容现有 `info`、`warning`、`error`、`danger` 及历史 `low`、`medium`、`high`，展示层统一映射，不重写历史数据。
...
可执行：跨赛事读取、按 Race / 状态 / 类型 / 严重度筛选和审计。

不可执行：修改风险状态、写处置说明或代替 Organizer 完成处置。
...
* 改写 `ReviewFlag` 状态。
* 读取内部来源字段、原始 CA 信息或未分配作品的风险。

### 6.5 非阻断规则

`open` 与 `in_review` 会进入相应角色的判断上下文，但不会自动阻止报名、Work 提交或评审；自动生成和解决只维护提示与审计闭环。

...
| attestation.source | enum | 是 | 上报来源，MVP 只接受 `ocr_desktop_app` 或正式登记的 CA connector 来源，不接受浏览器手填来源 |
| attestation.signingKeyId | string | 是 | ARY 为该 connector / OCR Desktop App 分配或登记的签名密钥 ID |
| attestation.signature | string | 是 | 对 canonical message payload 的签名摘要；MVP 本地演示使用 `dev-signature:` 前缀模拟，不代表生产密码学强度 |
| attestation.signedAt | datetime | 是 | 签名时间，用于防重放窗口判断 |
...
* `attestation.source` 不得来自普通浏览器表单、手工 JSON 或未登记脚本；本地 MVP 只接受 `ocr_desktop_app` 和 `registered_ca_connector`。
* `attestation.signingKeyId` 必须和 `CAConnection.connectorId` 对应的登记密钥匹配；生产实现应使用服务端保存的 connector 公钥或共享密钥进行验签。
* `attestation.signature` 必须覆盖 `messageId`、`idempotencyKey`、`race`、`rider`、`ca`、`signal`、`counters` 和 `timestamp` 的 canonical payload，避免字段被中途篡改。
* `idempotencyKey + messageId + sequence + signedAt` 共同用于幂等和防重放；重复消息只能幂等忽略，异常重复应进入隔离审计。
* 验签失败、缺少 attestation、来源不是 OCR Desktop App / 已登记 connector、签名密钥不匹配或时间窗口异常的消息，不得进入 Session、Evidence、Projection 或 Report，只能进入隔离审计和评审前风险提示。
* 本地 MVP 允许用 `dev-signature:<connectorId>:<idempotencyKey>` 模拟验签，以便展示反作弊流程；正式上线必须替换为真实 HMAC / 公私钥验签。
...
```json
{
  ...
  "attestation": {
    ...
    "signingKeyId": "ocr_key_codex_connector_001",
    "signature": "dev-signature:codex_connector_001:codex:race_2026_demo:reg_008:conn_codex_001:session_abc:seq_42",
    "signedAt": "2026-06-14T10:18:32Z"
  },
  "signal": {
    "kind": "note",
    "type": "task_progress",

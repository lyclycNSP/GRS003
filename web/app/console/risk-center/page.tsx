...
export default async function RiskCenterPage({
  ...
  const organizerFlags = sortFlags(allFlags.filter((item) => matchesFilter(item, filters)));
  ...
  const riderNames = [...new Set(allFlags.map((item) => item.riderName))].sort((left, right) => left.localeCompare(right));
  const typeOptions = [...new Set(allFlags.map((item) => item.type))].sort();
  const severityOptions = [...new Set(allFlags.map((item) => item.severity))].sort();
  const statusOptions = [...new Set(allFlags.map((item) => item.status))].sort();
  const canManage = ctx.roles.includes("admin") || ctx.managedRaceIds.includes(race.id);
  const openCount = allFlags.filter((item) => item.status === "open").length;
  const inReviewCount = allFlags.filter((item) => item.status === "in_review").length;
  const resolvedCount = allFlags.filter((item) => item.status === "resolved").length;
  const highRiskCount = allFlags.filter((item) => ["high", "danger"].includes(item.severity) && item.status !== "resolved").length;
  const missingMaterialCount = allFlags.filter((item) => item.type === "missing_required_material" && item.status !== "resolved").length;
  const ingestionCount = allFlags.filter((item) => item.type === "ingestion_exception" && item.status !== "resolved").length;

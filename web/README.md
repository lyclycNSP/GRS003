...
* Console：Organizer、Rider、Judge、Admin 角色入口，报名审核、CAConnection、Work、Judge、Award、Report、Projection、Admin roles。
* Ops：P0 回归、发布检查项、备份记录。
* API：`/api/public/races`、`/api/public/races/:slug`、Live、Works、Results、Review、Work detail、Rider detail。

DEV-2 / DEV-3 的高保真页面与交互已迁入 `web/`：公开浏览闭环、Profile Completion、Admin 角色维护、Organizer/Rider/Judge Console 入口均通过服务端数据和 Server Actions 驱动。
...
* Organizer View：报名审核、RaceProject 幂等生成、作品公开、Judge 分配、Award 发布、Report 生成/发布、Projection 重建。
...
* Ops：P0 回归、发布检查项、备份记录和运维入口。

### Data / APIs / Tests

* Prisma + SQLite 覆盖 User、AuthAccount、Race、Registration、RaceProject、CAConnection、Session、Work、Evidence、ReviewFlag、JudgeAssignment、JudgingRecord、Award、Report、Projection、Announcement、Backup、Incident、ReleaseChecklistItem。
* Public API 覆盖 races、race detail、live、works、results、review、work detail、rider detail。
* 领域测试覆盖重复报名、RaceProject 幂等、权限拒绝、Profile Completion、Admin roles、CA 合法/非法接入、Report 可见性和 P0 回归。

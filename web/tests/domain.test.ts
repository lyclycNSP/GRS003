...
async function main() {
  ...
  await test("Rider team can create, join, submit and share CA access", async () => {
    ...
    const memberWork = await submitWork(member, registration!.id, { title: "Member Work", summary: "Must be rejected" });
    assert.equal(memberWork.ok, false);
    const captainWork = await submitWork(captain, registration!.id, { title: "Team Flow Work", summary: "Submitted by captain" });

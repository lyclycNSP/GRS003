...
async function main() {
  ...
  assert.equal((await publishTrackProfileVersion(rider, { publishRequestId: "publish_denied_1", raceId: "race_bay_2026", profileJson: JSON.stringify(profile), background }, store)).ok, false);
  assert.equal((await publishTrackProfileVersion(managedRider, { publishRequestId: "publish_denied_role", raceId: "race_bay_2026", profileJson: JSON.stringify(profile), background }, store)).ok, false);
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_wrong_race", raceId: "race_finance_2026", profileJson: JSON.stringify(profile), background }, store)).ok, false);
  const first = await publishTrackProfileVersion(organizer, { publishRequestId: "publish_bay_track_1", raceId: "race_bay_2026", profileJson: JSON.stringify(profile), background }, store);
  assert.equal(first.ok, true);
  if (!first.ok) return;
  const repeated = await publishTrackProfileVersion(organizer, { publishRequestId: "publish_bay_track_1", raceId: "race_bay_2026", profileJson: JSON.stringify(profile), background }, store);
  assert.equal(repeated.ok, true);
  if (repeated.ok) assert.equal(repeated.id, first.id);
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_bay_track_1", raceId: "race_bay_2026", profileJson: JSON.stringify({ ...profile, version: "9.9.9" }), background }, store)).ok, false);
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_bay_track_1", raceId: "race_bay_2026", profileJson: JSON.stringify({ ...profile, name: "Changed Geometry Identity" }), background }, store)).ok, false);
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_bay_track_2", raceId: "race_bay_2026", profileJson: JSON.stringify(profile), background }, store)).ok, false);
  ...
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_bad_hash", raceId: "race_bay_2026", profileJson: JSON.stringify(tampered), background }, store)).ok, false);
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_bad_name", raceId: "race_bay_2026", profileJson: JSON.stringify({ ...profile, version: "1.0.2" }), background: new File([bytes], "../evil.webp", { type: "image/webp" }) }, store)).ok, false);
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_bad_mime", raceId: "race_bay_2026", profileJson: JSON.stringify({ ...profile, version: "1.0.3" }), background: new File([bytes], "background.svg", { type: "image/svg+xml" }) }, store)).ok, false);
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_big_profile", raceId: "race_bay_2026", profileJson: `${JSON.stringify(profile)}${" ".repeat(1024 * 1024)}`, background }, store)).ok, false);
  ...
  assert.equal((await publishTrackProfileVersion(organizer, { publishRequestId: "publish_large_geometry", raceId: "race_bay_2026", profileJson: JSON.stringify(excessiveGeometry), background }, store)).ok, false);

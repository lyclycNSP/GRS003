...
const admin = { ...organizer, availableRoles: ["admin" as const], activeRole: "admin" as const, managedRaceIds: [] };
...
async function main() {
  ...
  assert.equal((await pauseScreenRotation(rider, { raceId: "race_bay_2026", now: epoch })).ok, false);
  ...
  assert.equal((await configureScreenRotation(admin, { raceId: "race_finance_2026", intervalSeconds: 20 })).ok, false);
  assert.equal((await toggleScreenFallback(admin, "race_finance_2026", true)).ok, false);
  assert.equal((await configureScreenRotation(admin, { raceId: "race_finance_2026", intervalSeconds: 20, reason: "Emergency venue support" })).ok, false);

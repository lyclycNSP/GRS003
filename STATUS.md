...
* Rider、Organizer、Admin 和 Security E2E 已补齐并全量 13/13 通过；领域测试 22/22、静态烟测、TypeScript、PostgreSQL production build 和 production config preflight 同步通过。
...
| SEC-1 安全与生产基线验证通过：Playwright 13/13、领域测试 22/22、静态烟测、TypeScript、PostgreSQL client / production build、production config preflight 通过 | `web/e2e/security.spec.ts`、`web/tests/domain.test.ts`、`web/prisma/schema.prisma`、`web/scripts/check-production-config.mjs`、`docs/ary-production-security-baseline.md` |
...
| 浏览器自动化托管结果尚待首次推送确认 | 全角色 / Public / Screen / Security 13 个 E2E 和 CI 配置已在本地验证；推送后确认 GitHub-hosted Runner，并继续补截图基线、移动端和负载路径 |
